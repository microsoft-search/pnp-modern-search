...
export default class SearchFiltersWebPart extends BaseWebPart<ISearchFiltersWebPartProps> implements IDynamicDataCallables {
    ...
    private async renderAsync(): Promise<void> {
        ...
        if (!this.domElement) {
            Log.warn('SearchFiltersWebPart', 'domElement is not available, render aborted');
            return;
        }
    ...
    protected renderCompleted(): void {
        ...
        try {
            // Check if domElement is available before proceeding
            if (!this.domElement) {
                Log.warn('SearchFiltersWebPart', 'domElement is not available in renderCompleted');
                return;
            }
        ...
        super.renderCompleted();
            ...
            Log.error('SearchFiltersWebPart', new Error(`Error in renderCompleted: ${error}`));
            console.error('[SearchFiltersWebPart] Error in renderCompleted:', error);
        }
    }
    ...
    private async getConnectionOptionsFields(): Promise<IPropertyPaneField<any>[]> {
        ...
        const connectionFields: IPropertyPaneField<any>[] = [
    ...
    private getFilterSettings(): IPropertyPaneField<any>[] {
        ...
        if (this._dataSourceDynamicProperties) {
            ...
            this._dataSourceDynamicProperties.forEach(dataSourceDynamicProperty => {
                ...
                const dataResultsData = DynamicPropertyHelper.tryGetValueSafe(dataSourceDynamicProperty);
                if (dataResultsData) {
                ...
                fields: [
                        ...
                        onCustomRender: (field, value, onUpdate, item: IDataFilterConfiguration, itemId) => {
                            return React.createElement("div", { key: `${field.id}-${itemId}` },
                                React.createElement(Checkbox, {
                                    checked: item.selectedTemplate === BuiltinFilterTemplates.DateRange ? false : value,
                    ...
                    },
                    {
                        ...
                        onCustomRender: (field, value, onUpdate, item: IDataFilterConfiguration, itemId) => {
                            return React.createElement("div", { key: `${field.id}-${itemId}` },
                                React.createElement(Checkbox, {
                                    checked: item.selectedTemplate === BuiltinFilterTemplates.DateRange || item.selectedTemplate === BuiltinFilterTemplates.DateInterval ? false : value,
                    ...
                    {
                        id: 'useHierarchical',
                        title: webPartStrings.PropertyPane.DataFilterCollection.UseHierarchical,
                        type: this._customCollectionFieldType.custom,
                        defaultValue: false,
                        required: false,
        ...
        if (this.properties.useVerticals) {
            ...
            if (this.properties.verticalsDataSourceReference) {
                ...
                if (this._verticalsSourceData) {
                    const availableVerticals = DynamicPropertyHelper.tryGetValueSafe(this._verticalsSourceData);
    ...
    private mergeFiltersDataFromSources(dynamicProperties: DynamicProperty<IDataResultSourceData>[]): IDataResultSourceData {
        ...
        dynamicProperties.forEach(dataSourceDynamicProperty => {
            ...
            const dataSourceData = DynamicPropertyHelper.tryGetValueSafe(dataSourceDynamicProperty);
            if (dataSourceData) {
