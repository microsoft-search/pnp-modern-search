    ...
    */
    ...
    private async buildMicrosoftSearchQuery(dataContext: IDataContext): Promise<IMicrosoftSearchQuery> {
        ...
        if (this.properties.entityTypes.indexOf(EntityType.ListItem) !== -1) {
            ...
            // Build sort properties (only relevant for SharePoint managed properties)
            (dataContext.sorting?.selectedSorting ?? this.properties.sortProperties).filter(s => s.sortField).forEach(sortProperty => {
