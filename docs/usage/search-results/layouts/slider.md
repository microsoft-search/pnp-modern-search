...
| **Show page dots** | Enable or disable slider navigation. | True.
...
| **Enable details** | Show a **Details** action for selected items. Requires _Allow items selection_ to be enabled. When a single compatible item is selected, the action opens a right-side panel with the SharePoint details form for that item. When multiple selection is enabled and more than one compatible SharePoint item is selected, the same **Details** action opens a selected items edit panel instead. That panel only includes fields that are editable and common across the selected items, and unsupported selections can be excluded automatically. | False.
