...
| **Enable details** | Show a **Details** action for selected items. Requires _Allow items selection_ to be enabled. When a single compatible item is selected, the action opens a right-side panel with the SharePoint details form for that item. If multiple selection is enabled, the button can still appear when items are selected, but the action stays disabled until a single item is selected. | False.
