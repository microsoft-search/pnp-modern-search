...
| **Show warning when limit is reached** | Off by default. When enabled, the filter header shows a warning marker if the connected API returns a number of refiner or aggregation values equal to or greater than the configured **# of values** limit. Hover the marker to see the warning details. |
...
- The limit warning label and tooltip are localized using the current UI language for all supported Search Filters languages.
