...
A common need in any document library search experience is to let users access richer metadata about a result without navigating away from the search page. By adding a small info icon column to your results list, users can click it and a right-hand flyout panel appears showing metadata fields for that document.
...
The info panel column is added to the **DetailsList** layout, which presents results as a structured list of rows and columns. If your web part is not already using this layout:

1. Edit the web part and open the property pane
2. Under **Layouts**, select **Details List**

## Step 2: Add the required managed properties

Under **Data Source**, find the **Selected properties** field and ensure the following are included in addition to any you already have:
...
<pnp-panel   
    ...
    <template id="panel-open">
        <a href="#" title="More Info" style="text-decoration: none">
            <pnp-icon data-name="Info12" aria-hidden="true"></pnp-icon>
        </a>
    </template>
        ...
        <div class="docMetadata">
            <div style="display: flex; flex-wrap: wrap; gap: 0; margin-top: 16px; border-collapse: collapse; width: 100%;">
                <div style="display: flex; flex-direction: row; flex: 1 1 100%; padding: 8px 9px; border-bottom: none; align-items: center; font-weight: bold; color: black; text-align: left;">
                    <pnp-iconfile data-extension="{{slot item @root.slots.FileType}}" data-is-container="false" data-size="32" style="margin-right: 5px;"></pnp-iconfile>
                    {{slot item @root.slots.Filename}}
                </div>
